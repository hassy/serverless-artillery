...
const impl = {
  phaseQuestions: [
    ...
    },
    {
      type: 'input',
      name: 'rampTo',
      message() { return 'Ramp to load (in requests per second)'; },
      default: 10,
      validate(value) {
        return value > 0 ? true : 'Enter a positive value for ramp to load.';
      },
    },
    {
      type: 'confirm',
  ...
  scenarioQuestions: [
    {
      type: 'list',
      name: 'verb',
      message() { return `HTTP verb for scenario #${impl.wizardScript.scenarios.length + 1}`; },
      choices: [
        'GET',
        'POST',
  ...
  askScenario(done) {
    console.log();
    return inquirer.prompt(impl.scenarioQuestions)
      .then((scenarioAnswers) => {
        impl.wizardScript.scenarios.push(scenarioAnswers);

        if (scenarioAnswers.addAnother) {
          impl.askScenario(done);
        } else {
          done();
        }
      });
  ...
  scenarioWizard() {
    return impl.wizardLoop(impl.askScenario);
  },
  wizardLoop(ask) {
