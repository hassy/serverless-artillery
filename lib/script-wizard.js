...
const impl = {
  ...
  writeTestScriptFile() {
    const filename = impl.wizardScript.filename;
    const config = {
      target: impl.wizardScript.endpoint,
    };
    const phases = impl.wizardScript.phases;
    const scenarios = [{
      flows: [{
        requests: impl.wizardScript.requests,
      }],
    }];
    const testScript = scriptBuilder.buildScript(config, phases, scenarios);
